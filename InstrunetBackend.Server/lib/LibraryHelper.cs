...
public static class LibraryHelper
{
    ...
    /// </summary>
    /// <remarks>
    ///     This method determines the appropriate WebP encoder executable based on the operating
    ///     system and process architecture. If the platform or architecture is not supported, the method returns
    ///     <see
    ///         langword="null" />
    ///     .
    /// </remarks>
    /// <returns>
    ///     A <see cref="WebPEncoderBuilder" /> instance if the current platform and architecture are supported;
    ...
    /// </summary>
    /// <remarks>
    ///     The input stream's position is reset to its original value after processing. The
    ///     method processes the entire audio content synchronously and may be resource-intensive for large
    ///     files.
    /// </remarks>
    /// <param name="file">
    ///     The input stream containing MP3 audio data. The stream must be readable and positioned at the start of the
    ///     audio content.
    /// </param>
    /// <param name="pitch">
    ///     The number of semitones to shift the pitch. Positive values increase the pitch; negative values decrease it.
    ///     Fractional values are supported.
    /// </param>
    /// <returns>
    ///     A memory stream containing the pitch-shifted audio in MP3 format. The returned stream is positioned at the
    ///     beginning.
    /// </returns>
    public static MemoryStream ToPitched(this Stream file, double pitch)
    {
        try
        {
            ...
            int read;
            while ((read = p.Read(buffer, 0, buffer.Length)) > 0) wave.WriteSamples(buffer, 0, read);
            memoryStream.Position = 0;
            var outputMp3Stream = new MemoryStream();
            FFMpegArguments.FromPipeInput(new StreamPipeSource(memoryStream), o => o.WithAudioCodec("pcm_f32le"))
                .OutputToPipe(new StreamPipeSink(outputMp3Stream),
                    o => o.WithAudioCodec("mp3").ForceFormat("mp3").WithAudioBitrate(320000))
                .NotifyOnError(i => Console.WriteLine(i)).NotifyOnOutput(Console.WriteLine)
                .WithLogLevel(FFMpegLogLevel.Warning).ProcessSynchronously();
    ...
    /// <param name="pitch">
    ///     The amount of semitones by which to adjust the pitch, expressed as a multiplier. Values greater than 1.0 increase
    ///     the
    ///     pitch; values less than 1.0 decrease it.
    /// </param>
    ...
    public static MemoryStream ToPitched(this byte[] file, double pitch)
    {
        ...
        var mStreamProcessed = mStream.ToPitched(pitch);
        mStream.Dispose();
    ...
    public static (byte[] data, string fileName) MusicDecrypt(byte[] ncmFile, string predictFileName)
    {
        ...
        File.WriteAllBytes(execPath, Assembly.GetExecutingAssembly()
                                         .GetManifestResourceStream("InstrunetBackend.Server.lib.unlock_music.um.exe")
                                         ?.ReadToByteArray() ??
