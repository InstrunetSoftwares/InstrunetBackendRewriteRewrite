...
public static class MapUnlockMusicEndpoints
{
    public static RouteGroupBuilder MapSubmit(this RouteGroupBuilder app)
    {
        app.MapPost("/decrypterSubmit", ([FromBody] UnlockMusicSubmitPayload decrypterSubmitPayload) =>
        {
            ...
            try
            {
                File.WriteAllBytes($"{inputDir}/{uuid}_{decrypterSubmitPayload.FileName}",
                    decrypterSubmitPayload.FileInDataUri.DataUrlToByteArray());
                ...
                p.Start();
                p.WaitForExit();
                if (p.ExitCode != 0) return Results.BadRequest(new { message = "Decryption failed." });
    ...
    public static RouteGroupBuilder MapGetQqm(this RouteGroupBuilder app)
    {
        app.MapGet("/qqm", () =>
        {
            var stream = Assembly.GetExecutingAssembly()
                .GetManifestResourceStream("InstrunetBackend.Server.lib.QQMBinary.QQMusicSetup18.44.exe");
            if (stream is null) return Results.InternalServerError();
